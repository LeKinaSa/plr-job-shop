...
analyser(WorkingDirectory) :-
    ...
    enter_files, analyser(search_strategies   , [[], [restart], [first_fail], [occurrence], [most_constrained], [down], [first_fail, down], [occurrence, down], [most_constrained, down]]),
...
analyse(search_strategies, Value) :-
    analyse(75, 50, 4, 75, 50, 2, 80, 8, 15, Value).

analyse(NJobs, PercentAltJobs, NMachines, PercentAltMachines, AverageSizeTask, ProductionRange, TimeUsage, OverTimeHours, Timeout, SearchStrategies) :-
    % Clean Memory
    clean_memory,
    % Load File
    ...
    jobshop(TimeoutInMilliseconds, Branches, Conflicts, ObjValue, Status, WallTime, SearchStrategies),
